...
export const useRecords = (): UseRecordsReturn => {
  ...
  const addCalorieRecord = async (record: Omit<CalorieRecord, "date">) => {
    try {
      setLoading(true);
      await axios.post(
        `${config.backendAPIBaseUrl}/api/records/calorie/`,
        {
          ...record,
  ...
  const addSleepRecord = async (record: Omit<SleepRecord, "date">) => {
    try {
      setLoading(true);
      await axios.post(
        `${config.backendAPIBaseUrl}/api/records/sleep/`,
        {
          ...record,
          date: new Date().toISOString().split("T")[0],
        },
  ...
  const getCalorieRecords = async (
    ...
    try {
      ...
      const response = await axios.get<CalorieRecord[]>(
        `${config.backendAPIBaseUrl}/api/records/calorie/?start_date=${startDate}&end_date=${endDate}`,
        { withCredentials: true }
      );
      setError(null);
  ...
  const getSleepRecords = async (
    ...
    try {
      ...
      const response = await axios.get<SleepRecord[]>(
        `${config.backendAPIBaseUrl}/api/records/sleep/?start_date=${startDate}&end_date=${endDate}`,
        { withCredentials: true }
      );
      setError(null);
      return response.data;
